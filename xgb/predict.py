...
def main():
    ...
    keeneland_entries_path = os.path.join(DATA_DIR, 'keeneland_entries.csv')
    ...
    # Check if tomorrow's race data exists
    if os.path.exists(keeneland_entries_path):
        print("Loading tomorrow's race entries...")
        tomorrow_races = pd.read_csv(keeneland_entries_path)
    else:
        print(f"Warning: No data for tomorrow's races found at {keeneland_entries_path}")
        tomorrow_races = None
    ...
    # If we have tomorrow's races, make predictions
    if tomorrow_races is not None:
        print("\nPreparing tomorrow's race data for prediction...")
        # Preprocess tomorrow's data
        processed_tomorrow = preprocess_data(tomorrow_races, is_training=False)
        
        # Engineer features for tomorrow's data
        # This will calculate performance metrics for each horse based on historical data
        print("Analyzing horses' past performance in conditions similar to tomorrow's races...")
        feature_tomorrow = engineer_features(processed_tomorrow, historical_data=processed_data, is_training=False)
        
        # Group by race
        race_groups = feature_tomorrow.groupby(['race_date', 'track_code', 'race_number'], observed=True)
        
        # Make predictions for each race
        print("\nMaking predictions for tomorrow's races...")
        all_predictions = []
